...
output_dir="/scratch1/migriver_CCGP/ncbi_dataset/omnic_mapfiles"
...
for input_file_1 in "${input_dir}"/*_1.fastq; do
    # Extract the base name by removing "_1" and the extension
    base_name=$(basename "${input_file_1}" | sed 's/_1.*//')

    # Construct the paths for the input and output files
    input_file_2="${input_dir}/${base_name}_2.fastq"
    ...
    fastp --in1 "${input_file_1}" --out1 "${output_file1}" \
          --in2 "${input_file_2}" --out2 "${output_file2}" \
          --html "${html_report}" \
